...
const CARD_COLORS = ['#E21500', '#E5E7EB']
const SERVER_COLORS = ['rgb(124,77,255)', '#E5E7EB']
...
function InventoryDonut({ data, total, label, colors, schedule }: DonutProps) {
  return (
    <div className="rounded-xl border border-[#E2E8F0] bg-white p-5">
      <p className="mb-4 text-[11px] font-semibold uppercase tracking-[0.08em] text-[#9CA3AF]">{label}</p>
      <div className="flex items-center gap-5">
        <div className="relative w-28 h-28 flex-shrink-0">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              ...
              <Tooltip
                formatter={(v) => [`${v} units`, '']}
                contentStyle={{ background: '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: 8, color: '#111827', fontSize: 12 }}
              />
            </PieChart>
          </ResponsiveContainer>
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            <span className="text-[28px] font-semibold leading-tight text-[#111827]">{total}</span>
        ...
        <div className="flex-1 space-y-3">
          ...
          <div className="pt-2 border-t border-[#E2E8F0]">
            <p className="text-xs text-[#9CA3AF] mb-1.5">Upcoming POC</p>
            <div className="flex flex-wrap gap-1.5">
              {schedule.map((s) => (
                <span
                  ...
                  style={s.active
                    ? { background: `${colors[0]}22`, color: colors[0] }
...
export default function InventorySection() {
  return (
    <div>
      <h2 className="mb-3 text-[11px] font-semibold uppercase tracking-[0.08em] text-[#9CA3AF]">US Office Inventory</h2>
